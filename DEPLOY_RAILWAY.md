...
| Playwright `Executable doesn't exist` | Pin **`playwright==1.49.0`** trong `requirements-docker.txt` + image **`v1.49.0-jammy`** (cùng version). |
| `No module named 'playwright'` | Thêm lại `playwright==1.49.0` vào `requirements-docker.txt` — redeploy. |
| Telegram `409 Conflict` getUpdates | Hai process cùng token (máy local + Railway, hoặc 2 service). **Tắt** `bot_commands` trên PC, chỉ chạy Railway. |
